...
object QuickUIUtils {
    ...
    private fun createCustomAlert(title: String, message: Spanned, cancelable: Boolean, context: Context, center: Boolean = false, withAnimation: Boolean, redAlert: Boolean, iconTint: Int): Array<Any?> {
        ...
        val dialogView = if (redAlert) LayoutInflater.from(context).inflate(R.layout.custom_red_alert_dialog, null) else LayoutInflater.from(context).inflate(R.layout.custom_alert_dialog, null)
